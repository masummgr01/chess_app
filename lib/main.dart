...
class _ChessBoardScreenState extends State<ChessBoardScreen> {
  ...
  void _movePiece(Position from, Position to) {
    ...
    // Check for a king capture (simple win condition, since we skip check/checkmate)
    if (capturedPiece != null && capturedPiece.type == PieceType.king) {
      ...
      currentTurn = currentTurn; // freeze turn
      return;
    }
  ...
  // ---------- Move generation (simplified: no check detection) ----------
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Basic Chess')),
      body: Column(
        children: [
          Padding(
